...
//		Select * from tb_proveedor --> LISTA
		
		String jpsql = "Select p from Proveedor p";
		List<Proveedor> lstProveedor = em.createQuery(jpsql, Proveedor.class).getResultList();
...
//		Select * from tb_categorias --> LISTA
		...
		String jpsql = "Select c from Categoria c";
		List<Categoria> lstCategorias = em.createQuery(jpsql, Categoria.class).getResultList();
...
//		PROCESOS
		...
		p.setEst_prod(1);//Valor "default la registrar" 1->true / 0->false
		...
		try {
			...
			aviso("Error al registrar"+ e.getCause().getMessage());
		}
		em.close();
	}
